...
while(cap.isOpened()):
    ...
    combined_image = cv2.addWeighted(frame,0.8, line_image,1, 1)#(img1, weight(multplied with each pixel of the image),img2,weight of img2, gamma value )
    cv2.imshow('results',combined_image)
    if cv2.waitKey(1) & 0xFF == ord('q'): # bitwise and used for crossplatform safety
        break

cap.release() #close video file
cv2.destroyAllWindows() # close window we are currently on

    #now to break out of the for loop we want to close video on a click of a button